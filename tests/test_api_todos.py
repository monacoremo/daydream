...
def test_create_todo(service_endpoint, alice_session):
    ...
    current_user = alice_session.get(f"{service_endpoint}/api/rpc/current_user").json()[
        0
    ]

    resp = alice_session.post(
        f"{service_endpoint}/api/todos",
        json={"user_id": current_user["user_id"], "description": "Test todo",},
...
def test_todo_visibility(service_endpoint, alice_session, bob_session):
    ...
    resp = alice_session.post(
        f"{service_endpoint}/api/todos",
        json={"user_id": current_user["user_id"], "description": todo_desc,},
        headers={"Prefer": "return=representation"},
    )
    assert resp.status_code == 201
...
def test_create_todo_anonymous(service_endpoint, alice_session):
    ...
    resp = alice_session.post(
        f"{service_endpoint}/api/todos",
        json={"user_id": 1, "description": "Test todo",},
    )
